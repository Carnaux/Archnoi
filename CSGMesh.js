...
class CSG {
    ...
    subtract(csgB) {
        var a = new Node(this.clone().polygons);
        var b = new Node(csgB.clone().polygons);
        a.invert();
        a.clipTo(b);
        b.clipTo(a);
        b.invert();
    ...
    ieval(tokens,index=0){
        if(typeof tokens === 'string')
            this.currentOp=tokens;
        else if(tokens instanceof Array){
            for(let i=0;i<tokens.length;i++)this.ieval(tokens[i],0);
        }else if(typeof tokens==='object'){
            var op=this.currentOp;
            tokens.updateMatrix();
            tokens.updateMatrixWorld();
            if(!this.sourceMesh)
                this.currentPrim =  this.fromMesh(this.sourceMesh = tokens);
            else{
                this.nextPrim = this.fromMesh(tokens);
                this.currentPrim = this.currentPrim[op](this.nextPrim);
            }
            if(this.doRemove)tokens.parent.remove(tokens);
        }//union,subtract,intersect,inverse
    }

    eval(tokens,doRemove){//[['add',mesh,mesh,mesh,mesh],['sub',mesh,mesh,mesh,mesh]]
        this.doRemove=doRemove;
        this.ieval(tokens);
        var result = this.toMesh( this.currentPrim, this.sourceMesh.matrix );
        result.material = this.sourceMesh.material;
